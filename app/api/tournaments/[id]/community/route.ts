...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [slot, approvedPayment] = await Promise.all([prisma.participant.findUnique({ where: { tournamentId_userId: { tournamentId: id, userId: user.id } } }), prisma.payment.findFirst({ where: { tournamentId: id, userId: user.id, status: 'VERIFIED' } })]);
